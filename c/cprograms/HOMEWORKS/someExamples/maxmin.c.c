...
void maximum(int n){
    int maximum=bhishan[0].oh;     //suppose first number of array is maximum
    int i;

        for(i=1;i<n;i++){
            if(bhishan[i].oh>maximum)  //ith number is greater than first
                maximum=bhishan[i].oh;  //then maximum is ith number
            else
                maximum=maximum;
        }
