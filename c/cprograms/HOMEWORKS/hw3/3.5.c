...
printf("***********************************************************\n");
	...
	{ 
		...
		if (time>=800 && time<=1800 )
		{
			...
			printf("For business days call rate is 33 cents per minute for peak hours 8am thru 6pm\n");	
			printf("Total due is = $%.2f\n", cost); 
		}
		else
		{	cost = (length * 0.21);				// 21 cents per minutes.
			printf("For business days call rate is 21 cents per minute for rest hours 6pm thru 8am\n");	
			printf("Total due is = $%.2f\n", cost); 
		}
