// topic     : cookbook
// to compile: gcc cookbook.c -O2 -lcfitsio -lm -pthread
// to run    : ./a.out
// terminal  : clear; gcc cookbook.c -O2 -lcfitsio -lm -pthread && ./a.out
    ...
    //*****************************************************************//
{
    ...
    fits_write_col(fptr, TLONG, 2, firstrow, firstelem, nrows, diameter, &status);
    fits_write_col(fptr, TFLOAT, 3, firstrow, firstelem, nrows, density, &status);
...
{
    fitsfile        *infptr;
    fitsfile        *outfptr;  // pointer to input and output FITS files //
