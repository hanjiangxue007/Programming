...
if __name__ == '__main__':
    ...
    for i in list(range(1)):
        ...
        ingal = 'gal{:d}.fits'.format(i)
        replace_galfit_param('A', ingal, object_num=1, fit=True)

        # run galfit
        cmd = 'galfit -o3 sim.feedme'
        subprocess.call(cmd,shell=True)

        # get model and residue
        model = 'outputs/gal_out{:d}.fits'.format(i)
        res   = 'outputs/gal_res{:d}.fits'.format(i)

        data1,header1 = fits.getdata(r"subcomps.fits", ext=1, header=True)
        data2,header2 = fits.getdata(r"subcomps.fits", ext=2, header=True)
