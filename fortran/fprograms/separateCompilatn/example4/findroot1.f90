...
!cmd   : clear; gfortran -Wall findroot1.f90 && ./a.out
!cmd   : clear; f90 findroot1.f90 && ./a.out
...
!        call the bisection routine
      ...
      call bisect1 (xinitial,xfinal,root,function_x)  ! root is output all rest are inputs
...
!        starting from xinitial
 ...
 61   ie=ie+1    ! now, ie = 2, this is like do loop
      ...
      if (ie.gt.iemax) stop  ! iemax = max iterations = 30
          ...
          call newtonr (fxx(ie-1),fxx(ie),xx(ie-1),xx(ie),xx(ie+1),delta) ! fxx(1), fxx(2), xx(1), xx(2),delta
