...
run_process("colour.c",['./cl',"color2.txt",str(0.5)])
...
for x in range(0,21):
 ...
 # os.makedirs("simdatabase/f1")  
  ...
  run_process("colour.c",['./cl',"color2.txt",str(x)])

  #t[x] = config['output_folder'] + config['prefix'] + t[x]   
  ...
  run_process("jedidistort", ['./dis1', config['nx'], config['ny'], config['dislist_file'], config['lenses_file'], config['pix_scale'], config['lens_z']])
...
#for varying pf in 90 degree case
m=[]
...
run_process("colour2.c",['./cl',"color2.txt",str(0.5) ])
...
for l in range(0,21):
  ...
  run_process("colour.c",['./cl',"color2.txt",str(l) ])
